...
def rollout(
    ...
    return Rollout(completion, prompt_mask)


def log_probs(model: PreTrainedModel, rollout: Rollout) -> torch.Tensor:
    ...
    response_tokens_mask = torch.tensor(rollout.prompt_mask[1:])  # T
...
def loss(
    ...
    response = tokenizer.decode(rollout.response, skip_special_tokens=True)
    reward = environment.reward(response)
    print(f"Reward: {reward:.4f}; Response: {response}")

    log_probs_sum = log_probs(model, rollout).sum(dim=-1)  # 1
...
def train(
    ...
    for step in range(TRAIN_STEPS):
        ...
        model.train()
        losses = [loss(model, tokenizer, environment, rollout) for rollout in rollouts]
        ...
        print(f"Step {step + 1}/{TRAIN_STEPS}, Loss: {mean_loss.item():.4f}")
