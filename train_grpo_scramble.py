...
#     get_scheduler,
# )
# from reward import scramble_reward
...
# ## A simpler task for smaller models... 0.6B can't do this, 1.7B can out-of-the-box.
...
#     def __init__(self) -> None:
#         self._dictionary_parquet = pq.ParquetFile(self.DICTIONARY_PARQUET_PATH)

#     def iter_batches(self, batch_size: int) -> Iterator[list[str]]:
#         for batch in self._dictionary_parquet.iter_batches(batch_size=batch_size):
#             batch_dicts = batch.to_pylist()
#             rows = [row_dict["word"] for row_dict in batch_dicts]
#             yield rows
...
#         {
#             "role": "system",
#             "content": "Your task is to rearrange the letters in a word. For example, given the word 'tiger', you could return 'irgte'. You must ONLY return the rearranged word and NOTHING ELSE.",
#         },
#         {
#             "role": "user",
#             "content": f"Rearrange the letters in the following word: `{prompt}`",
#         },
#     ]
...
#         reward = scramble_reward(prompt_text, response_text)
...
#                 loss = rollout_group_loss(tokenizer, model, group)
#                 loss.backward()
#                 optimizer.step()
#                 lr_scheduler.step()
#                 optimizer.zero_grad()
